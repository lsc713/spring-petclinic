...
public interface ChaosFaults {
	 ...
	 * Under the {@code oomKill} scenario it returns {@code true}, so the endpoint allocates
	 * and retains heap beyond the pod memory limit until the kernel OOM-kills the
	 * container — a SIGKILL with no application stack trace.
